...
def cleanstring(unclean_string):
	...
	clean_string = split_unclean_string[1][index_last_backslash+1:index_last_quote]
	return clean_string
...
IN_pipeline_name = input('Give the precise name of ADF Pipeline you want to deploy: ')

while True:
	IN_datafactory_flag = input('For deployment to ADF Dev insert "dev", to ADF Live type in "live", to ADF Live-Replica type in "replica" : ')
...
all_resources_list = findalldependencies(datafactory_resources,IN_pipeline_name)
...
if data_factory_name == 'BIDataFactoryLive-REPLICA':
	...
	datafactory_parms["parameters"]["LD9BA1-IR_properties_typeProperties_linkedInfo_resourceId"] = {"value": newmapping["LD9BA1-IR_properties_typeProperties_linkedInfo_resourceId"]["defaultValue"]}
	for item in datafactory_resources:
...
prod_datafactory_string = replacer(datafactory_string,values_to_replace_list,newmapping)
prod_datafactory_parms_string = replacer(datafactory_parms_string,values_to_replace_list,newmapping)
...
Write to file
'''
NewDataFactoryDeploymentFile = open("NewDataFactoryDeploymentFile_{}.json".format(data_factory_name),"w")
NewDataFactoryDeploymentFile_parms = open("NewDataFactoryDeploymentFile_{}_parms.json".format(data_factory_name),"w")
